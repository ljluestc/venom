...
type Executor struct {
	...
	DNS               []string          `json:"dns" yaml:"dns" mapstructure:"dns"` // Added field for custom DNS servers
...
func (Executor) Run(ctx context.Context, step venom.TestStep) (interface{}, error) {
	...
	if len(e.DNS) > 0 {
		...
		resolver := &net.Resolver{
			...
			Dial: func(ctx context.Context, network, address string) (net.Conn, error) {
				...
				return d.DialContext(ctx, "udp", e.DNS[0]+":53")
			},
		}
		dialer = &net.Dialer{
			Resolver:  resolver,
	...
	if len(e.Resolve) > 0 && (len(e.UnixSock) > 0 || len(e.DNS) > 0) {
		return nil, fmt.Errorf("you can't use 'resolve' with 'unix_sock' or 'dns' attributes at the same time")
	}
	...
	if len(e.Resolve) > 0 {
		...
		// Use the configured dialer (system or custom DNS)
